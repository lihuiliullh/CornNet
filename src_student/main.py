...
from model_teacher_student import *
from model_student import *

sys.path.append("./")

from logger import setup_logger
from deprecated_model_LSTM import *
...
def validate_v2(config, model, data_loader, device, num_entities, output_rank=False):
    ...
    for i_batch, data in enumerate(loader):
        ...
        q_text4 = data[19]
        q_text5 = data[20]
        answers = model.get_score_ranked2(head, q1, m1, a1, 
        q2, m2, a2,
        q3, m3, a3,
        q4, m4, a4,
        q5, m5, a5,
...
def train_again(config, model, optimizer, scheduler, dataloader, valid_data_loader, test_data_loader, data_loader_LLM, valid_data_loader_LLM, test_data_loader_LLM, device, num_entities, max_acc):
    #validate_v2(config=config, model=model, data_loader = test_data_loader_LLM, device=device, num_entities=num_entities, output_rank=True)
    ...
    for epoch in range(config.al_epochs):
        ...
        dataloader_iterator = iter(data_loader_LLM)
        ...
        for i_batch, data in enumerate(loader):
            ...
            data2[0].to(device), data2[1].to(device), data2[2].to(device), data2[3].to(device),
            data2[4].to(device), data2[5].to(device), data2[6].to(device),
            data2[7].to(device), data2[8].to(device), data2[9].to(device),
            data2[10].to(device), data2[11].to(device), data2[12].to(device),
            data2[13].to(device), data2[14].to(device), data2[15].to(device),
            data2[21].to(device), data2[22].to(device), data2[23].to(device), data2[24].to(device), data2[25].to(device), 
            )
            loss.backward()
            optimizer.step()
        ...
        if epoch % 3 == 0 and epoch != 0:
            ...
            model.eval()
            print("evaluate")
            valid_res = validate_v2(config=config, model=model, data_loader = valid_data_loader_LLM, device=device, num_entities=num_entities)
            if best_hit1 < valid_res[-1]:
                best_hit1 = valid_res[-1]
                best_res = valid_res
                # run test
                print("-----------------TEST--------------------------")
                test_res = validate_v2(config=config, model=model, data_loader = test_data_loader_LLM, device=device, num_entities=num_entities)
                print(test_res)
                # save model
...
def main():
    ...
    ##### for generated reformulation by LLMs
    if config.sample_strategy == 'kmeans':
        train_batcher_LLM = ActiveKMeansSelector(config.conversation_path_gpt2, kg_node_embeddings, config.entity2id, 
        config.sample_size, config.cluster_num, config)
    else:
        train_batcher_LLM = ActiveRandomSelector(config.conversation_path_gpt2, config.sample_size, config)
    ...
    ##### for generated reformulation by LLMs
    valid_batcher_LLM = ActiveRandomSelector(config.conversation_valid_path_gpt2, 999999, config)
    valid_data_LLM = valid_batcher_LLM.next()
    valid_conv_dataset_LLM = DatasetConversation(valid_data_LLM, config.entity2id, config.relation2id, config.entity2neighbor_relation, config.ht2relation)
    valid_data_loader_LLM = DataLoader(valid_conv_dataset_LLM, batch_size=config.test_batch_size, shuffle=False, num_workers=config.num_workers)

    test_batcher_LLM = ActiveRandomSelector(config.conversation_test_path_gpt2, 999999, config)
    test_data_LLM = test_batcher_LLM.next()
    test_conv_dataset_LLM = DatasetConversation(test_data_LLM, config.entity2id, config.relation2id, config.entity2neighbor_relation, config.ht2relation)
    test_data_loader_LLM = DataLoader(test_conv_dataset_LLM, batch_size=config.test_batch_size, shuffle=False, num_workers=config.num_workers)
    ...
    reformulation_imitator = ReformulationImitator(config, embedding_dim=config.embedding_dim, num_entities = len(config.entity2id), relation_emb = relation_embeddings, 
        pretrained_embeddings=kg_node_embeddings, 
        freeze=config.freeze, device=config.device, entdrop = config.entdrop, reldrop = config.reldrop, scoredrop = config.scoredrop, 
        l3_reg = config.l3_reg, ls = config.ls, do_batch_norm=config.do_batch_norm)
    
    # config.load_model should be true here, because the teacher model has been trained
    if config.load_model:
        model.load_state_dict(torch.load(config.model_save_path))
    
    model.set_reformulation_imitator(reformulation_imitator)
    ...
    if config.reranking:
        ...
        for i in range(config.active_round):
            print("###################################################")
            new_data = train_batcher.next()
            new_data_LLM = train_batcher_LLM.next()
            if len(new_data) <= 0:
                print("end")
                break
            conv_dataset = DatasetConversation(new_data, config.entity2id, config.relation2id, config.entity2neighbor_relation, config.ht2relation)
            ...
            ##### for generated reformulation by LLMs
            conv_dataset_LLM = DatasetConversation(new_data_LLM, config.entity2id, config.relation2id, config.entity2neighbor_relation, config.ht2relation)
            data_loader_LLM = DataLoader(conv_dataset_LLM, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers)
            
            max_acc = train_again(config, model, optimizer, scheduler, data_loader, valid_data_loader, test_data_loader, data_loader_LLM, valid_data_loader_LLM, test_data_loader_LLM, device, len(config.entity2id), max_acc)
